...
export default function AnimatedIcons() {
  ...
  return (
    <motion.div ref={ref} className="absolute overflow-hidden w-full flex flex-row items-start left-0 md:-translate-y-64 -translate-y-32">
      <motion.div
      ...
      >
        {iconList.concat(iconList).concat(iconList).concat(iconList).concat(iconList).concat(iconList).concat(iconList).map((icon, index) => (
          <img
            ...
            height={150}
            loading="eager"
            className="md:size-[64px] size-[36px] mx-0 md:mx-[0vh]"
            alt={icon.alt}
            src={icon.src}
          />
        ))}
